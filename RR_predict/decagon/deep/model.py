...
from .layers import GraphConvolutionSparseMulti, \
    DistMultDecoder, InnerProductDecoder, DEDICOMDecoder, BilinearDecoder, GraphAttentionMulti, GraphConvolutionMulti
...
class DecagonModel(Model):
    def __init__(self, placeholders, num_feat, nonzero_feat, edge_types, decoders, encoder='gat', **kwargs):
        ...
        # self.adj_bias = {n: [
        #     placeholders['adj_bias_%d' % n]] for n in range(2)}
        self.build()
    ...
    def _build(self):
        ...
        for i, j in self.edge_types:
            if self.encoder == 'gcn':
                ...
                self.embeddings_reltyp[i].append(GraphAttentionMulti(
                    input_dim=FLAGS.hidden1, output_dim=FLAGS.hidden2,
                    edge_type=(i, j), num_types=self.edge_types[i, j],
