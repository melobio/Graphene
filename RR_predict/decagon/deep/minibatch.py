...
class EdgeMinibatchIterator(object):
    ...
    """
    def __init__(self, adj_mats, adj_bias, feat, edge_types, batch_size=100, val_test_size=0.01):
        ...
        self.adj_bias_all = {0: None, 1:None}
        for i, j in self.edge_types:
            self.adj_bias_all[j] = preprocessing.sparse_to_tuple(self.adj_bias[j][0])
            for k in range(self.edge_types[i,j]):
    ...
    def end(self):
        ...
        a = self.edge_type2idx[1, 1, 0] not in self.freebatch_edge_types
        b = self.edge_type2idx[1, 1, 1] not in self.freebatch_edge_types
        if a & b:
            finished = 1
        else:
            finished = 0
    ...
    def update_feed_dict(self, feed_dict, dropout, placeholders):
        ...
        # feed_dict.update({
        #     placeholders['adj_bias_%d,%d' % (i, j)]: self.adj_bias_all[i, j] for i, j in self.edge_types})
