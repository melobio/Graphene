...
def calculate_Sab(set_a, set_b, net_G, all_genes_in_network):
    ...
    d_A = calc_single_set_distance(net_G, genes_A)
    d_B = calc_single_set_distance(net_G, genes_B)

    # distances BETWEEN the two gene sets:
    d_AB = calc_set_pair_distances(net_G, genes_A, genes_B)
...
f = open(r"E:\projects\gat decagon\Disease_gene_prioritization_GCN\comorbidity\disease_gen.json","r")
new_dict = json.load(f, encoding='utf-8')

# set1 = set(new_dict["dyskinesias"][1])
# print(len(set1))
# set2 = set(new_dict["movement disorders"][1])
# print(len(set2))
# ress = calculate_Sab(set1, set2)
# print(ress)
# print(-ress)

df1 = pd.read_csv(r"E:\projects\gat decagon\Disease_gene_prioritization_GCN\comorbidity\Disease pair and comorbidity.csv")

#print(df1.iloc[0]["Disease Pair"].strip().split("&"))
pos_ind = (random.sample(range(0,6269),1200))
neg_ind = (random.sample(range(6270,10743),1200))
pred_pos = []
pred_neg = []
count1 = 0
for i in pos_ind:
    dis1 = df1.iloc[i]["Disease Pair"].strip().split("&")[0]
    dis2 = df1.iloc[i]["Disease Pair"].strip().split("&")[1]
    set1 = set(new_dict[dis1][1])
    set2 = set(new_dict[dis2][1])
    pred = -calculate_Sab(set1, set2, G, all_genes_in_network)
    pred_pos.append(pred)
    count1 += 1
    print(count1)
count2 = 0
for j in neg_ind:
    dis1 = df1.iloc[j]["Disease Pair"].strip().split("&")[0]
    dis2 = df1.iloc[j]["Disease Pair"].strip().split("&")[1]
    set1 = set(new_dict[dis1][1])
    set2 = set(new_dict[dis2][1])
    pred = -calculate_Sab(set1, set2, G, all_genes_in_network)
...
np.save("./s_ab_result/s_ab_preds.npy",preds_all)
np.save("./s_ab_result/s_ab_labels.npy",labels_all)