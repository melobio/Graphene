...
def main(args):
    ...
    # initialize graph
    dur = []
    input_ids = torch.linspace(0, 17538, 17539).long().to(args.gpu)
    best_val = 0
    for epoch in range(args.epochs):
        ...
        if epoch % 1000 == 0 and val_roc > best_val:
            best_val = val_roc
            # 保存模型
            ...
            np.save("./data/predicts.npy", predicts_nmupy)
            np.save("./data/labels.npy", labels_nmupy)
