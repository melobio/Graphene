...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        #if epoch % 40 == 0:
            # 保存模型
            torch.save(model.state_dict(), './data/ablation_checkpoint/ablation_epoch{:05d}_valacc{:.4f}_val_roc{:.4f}_checkpoint.pt'.format(epoch,val_acc,val_roc))
            # 保存矩阵
            predicts_nmupy = logits.transpose(1, 0).cpu().detach().numpy()
            labels_nmupy = labels.transpose(1, 0).cpu().detach().numpy()
            np.save("./data/ablation_checkpoint/predicts.npy", predicts_nmupy)
            np.save("./data/ablation_checkpoint/labels.npy", labels_nmupy)
            end = time.clock()
            runTime = end - start
            print("1000_epoch时间：", runTime, "秒")
            


    if args.early_stop:
        model.load_state_dict(torch.load('./data/ablation_checkpoint/ablation_checkpoint.pt'))
