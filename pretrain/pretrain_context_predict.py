...
def train(args, model_substruct, model_context, loader, optimizer_substruct, optimizer_context, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        pos_loss_accum += float(loss_pos.detach().cpu().item())
        neg_loss_accum += float(loss_neg.detach().cpu().item())
    ...
    print("pos loss:", pos_loss_accum/step)
    print("neg loss:", neg_loss_accum / step)
...
def main():
    ...
    #set up dataset
    # root_unsupervised = '/home/sunzijun/pretrain-gnn/bio/dataset/unsupervised'
    ...
    datasets = torch.load("./new_dataloader/sub_data.pt")
    loader = DataLoaderSubstructContext(datasets, batch_size=5, shuffle=False)

    # loader = DataLoaderSubstructContext(dataset, batch_size=args.batch_size, shuffle=False, num_workers = args.num_workers)
    ...
    if not args.model_file == "":
        torch.save(model_substruct.state_dict(), args.model_file + "_0.pth")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if not args.model_file == "":
            torch.save(model_substruct.state_dict(), args.model_file + "_{}.pth".format(epoch))
